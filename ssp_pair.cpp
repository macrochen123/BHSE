...
char* PFC::finish_hash_to_char(void)
{
	char s[HASH_LEN];
	shs256_hash(&SH,s);
	return s;
}
void PFC::finish_hash_to_char1(char* result)
{	
	char s[HASH_LEN];
	shs256_hash(&SH,s);
	memcpy(result, s, HASH_LEN);
}
...
{ // compress it and add
	...
	to_binary(a, 192, cBig, true);
	for ( i = 0; i < 192; i++)
	{
		shs256_process(&SH, cBig[i]);
	}
   /* while (a>0)
    {
        m=a%256;
...
void PFC::add_to_hash(int ct)
{
	char pbDes[4] = {0};
	int i = 0;
	pbDes[0] = (char)((unsigned int)ct >> 24);
	pbDes[1] = (char)((unsigned int)ct >> 16);
	pbDes[2] = (char)((unsigned int)ct >> 8);
	pbDes[3] = (char)((unsigned int)ct);
	for ( i = 0; i < 4; i++)
	{
		shs256_process(&SH, pbDes[i]);
	}

}
void PFC::add_to_hash(char* keyword, int keyLen, int number)
{
	int i;
	char pbDes[4] = { 0 };
	for (i = 0; i < keyLen; i++)
	...
	pbDes[0] = (char)((unsigned int)number >> 24);
	pbDes[1] = (char)((unsigned int)number >> 16);
	pbDes[2] = (char)((unsigned int)number >> 8);
	pbDes[3] = (char)((unsigned int)number);
	for (i = 0; i < 4; i++)
	{
		shs256_process(&SH, pbDes[i]);
	}
...
Big PFC::hash_to_Big(char *ID)
{
	Big m=H1(ID);
	return m;
}
char* PFC::hash_to_Char(char *string)
{
	char s[HASH_LEN];
	int i; 
	sha256 sh;
