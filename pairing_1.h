...
// creates a smaller table
	...
	Big hash_to_Big(char *);
	char* hash_to_Char(char *);
	void finish_hash_to_char1(char* result);
...
// parameters: number of pairings n, pointers to pair of G1 elements
	...
	void add_to_hash(int ct);
	void add_to_hash(char* keyword, int keyLen, int number);
	Big finish_hash_to_group(void);
	char* finish_hash_to_char(void);
	~PFC() {
#ifdef MR_PAIRING_SSP
