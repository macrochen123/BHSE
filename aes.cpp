...
S32 AES_E(U8* Cipher,U8 *Message,U8 *Key,S32 M_Len)
{
	...
	if(padding!=0)//需要填充
	{
		for(i  = 0;i<MAX_BLOCK_SIZE;i++)
		{
			if(i<padding)
			{
				temp[i] = (S8)Message[(block_m*MAX_BLOCK_SIZE)+i];//获取消息剩余部分
				continue;
			}
			temp[i] = 0;//填充消息
		}
		aes_encrypt(&a,temp);//加密最后一块

		for (j=0;j<16;j++) 
		{
			Cipher[block_m*16+j]=(U8)temp[j];
		}
	}
	...
	for(i = 12; i < MAX_BLOCK_SIZE; i++ )
	{
		buff = (S8)(M_Len >>(MAX_BLOCK_SIZE-i-1)*8);
		temp[i] = buff;
	}

	aes_encrypt(&a,temp);//加密明文长度
	if(padding != 0)
	{
		for(j=0;j<16;j++) 
		{
			Cipher[(block_m+1)*16+j]=(U8)temp[j];
		}
	}
